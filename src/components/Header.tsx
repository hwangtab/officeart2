...
const navItemsResponsive: ResponsiveNavItem[] = [
  ...
  { name: '프리미엄 의자', shortName: '의자', href: '/premium-chairs', priority: 'high' },
  { name: '집중 환경', shortName: '환경', href: '/focus-environment', priority: 'high' },
  { name: '창작자 커뮤니티', shortName: '커뮤니티', href: '/creator-community', priority: 'high' },
  { name: '가격 및 멤버십', shortName: '가격', href: '/pricing', priority: 'high' },
  { name: '오시는 길', shortName: '위치', href: '/location', priority: 'medium' },
  { name: '비상주 사무실', shortName: '비상주', href: '/services/non-resident', priority: 'low' },
            ...
            {highPriorityItems.map((item) => {
              ...
              return (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className={`px-1 py-1 text-xs rounded-md transition-colors duration-150 whitespace-nowrap focus:ring-2 focus:ring-offset-2 focus:ring-offset-primary focus:ring-white ${
                      active
                        ? 'font-bold'
            ...
            {desktopNavItems.map((item) => {
              ...
              return (
                <li key={item.name}>
                  <Link
                    href={item.href}
                    className={`px-2 py-2 text-sm rounded-md transition-colors duration-150 whitespace-nowrap focus:ring-2 focus:ring-offset-2 focus:ring-offset-primary focus:ring-white ${
                      active
                        ? 'font-bold'
                        : 'font-medium hover:opacity-80'
                    }`}
