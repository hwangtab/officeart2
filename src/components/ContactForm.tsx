...
export default function ContactForm() {
  ...
  const onValid: SubmitHandler<FormData> = useCallback((data) => {
    ...
    const templateParams = {
      ...
      // Handle cases where interest might not be an array
      service: Array.isArray(data.interest)
        ? data.interest.join(', ')
        : data.interest || '미선택',
    };
  ...
  return (
    <Card className="mb-16 shadow-lg"> {/* Removed bg, p, rounded */}
      ...
      <form onSubmit={handleRHFSubmit(onValid)} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6 border-t pt-6 border-border-light">
          <div>
            <label htmlFor="visitDate" className="block text-sm font-medium mb-1 text-text-secondary">방문희망일 <span className="text-warning-red">*</span></label>
            <input
        ...
        <div>
          <label className="inline-flex items-center cursor-pointer">
            ...
            <button type="button" onClick={() => alert('개인정보처리방침 상세 내용 표시 기능 구현 예정')} className="ml-2 text-xs text-accent-blue hover:underline">(내용보기)</button>
          </label>
          {/* Error Message */}
        ...
        <div className="text-center pt-4 pb-4"> {/* 상단 패딩 복구, 하단 패딩 추가 */}
          <button
            type="submit"
            className="bg-primary hover:bg-orange-500 text-text-on-primary font-bold py-3 px-10 rounded-lg text-lg transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed" // Use primary color
